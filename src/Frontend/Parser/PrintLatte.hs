...
instance Print (AbsLatte.TopDef' a) where
  prt i = \case
    ...
    AbsLatte.ClsDef _ id_ clsinh clselemdefs -> prPrec i 0 (concatD [doc (showString "class"), prt 0 id_, prt 0 clsinh, doc (showString "{"), prt 0 clselemdefs, doc (showString "}")])
...
instance Print (AbsLatte.ClsInh' a) where
  prt i = \case
    AbsLatte.NoInh _ -> prPrec i 0 (concatD [])
    AbsLatte.Inh _ id_ -> prPrec i 0 (concatD [doc (showString "extends"), prt 0 id_])

instance Print (AbsLatte.Arg' a) where
  prt i = \case
    AbsLatte.VArg _ type_ id_ -> prPrec i 0 (concatD [prt 0 type_, prt 0 id_])
...
instance Print (AbsLatte.ClsElemDef' a) where
  prt i = \case
    AbsLatte.MetDef _ type_ id_ args block -> prPrec i 0 (concatD [prt 0 type_, prt 0 id_, doc (showString "("), prt 0 args, doc (showString ")"), prt 0 block])
    AbsLatte.AtrDef _ type_ id_ -> prPrec i 0 (concatD [prt 0 type_, prt 0 id_, doc (showString ";")])
...
instance Print (AbsLatte.Block' a) where
  prt i = \case
    AbsLatte.BBlock _ stmts -> prPrec i 0 (concatD [doc (showString "{"), prt 0 stmts, doc (showString "}")])
...
instance Print (AbsLatte.Stmt' a) where
  prt i = \case
    ...
    AbsLatte.For _ type_ id_ expr stmt -> prPrec i 0 (concatD [doc (showString "for"), doc (showString "("), prt 0 type_, prt 0 id_, doc (showString ":"), prt 0 expr, doc (showString ")"), prt 0 stmt])
...
instance Print (AbsLatte.Type' a) where
  prt i = \case
    AbsLatte.Var _ -> prPrec i 0 (concatD [doc (showString "var")])
    AbsLatte.Arr _ type_ -> prPrec i 0 (concatD [prt 0 type_, doc (showString "[]")])
    AbsLatte.Cls _ id_ -> prPrec i 0 (concatD [prt 0 id_])
    AbsLatte.Void _ -> prPrec i 0 (concatD [doc (showString "void")])

instance Print (AbsLatte.Expr' a) where
  prt i = \case
    AbsLatte.ECast _ id_ expr -> prPrec i 7 (concatD [doc (showString "("), prt 0 id_, doc (showString ")"), prt 6 expr])
    AbsLatte.EVar _ id_ -> prPrec i 6 (concatD [prt 0 id_])
    AbsLatte.ELitInt _ n -> prPrec i 6 (concatD [prt 0 n])
    AbsLatte.ELitNull _ -> prPrec i 6 (concatD [doc (showString "null")])
    AbsLatte.ELitTrue _ -> prPrec i 6 (concatD [doc (showString "true")])
    AbsLatte.ELitFalse _ -> prPrec i 6 (concatD [doc (showString "false")])
    AbsLatte.EArrAcs _ expr1 expr2 -> prPrec i 6 (concatD [prt 6 expr1, doc (showString "["), prt 0 expr2, doc (showString "]")])
    AbsLatte.EApp _ expr exprs -> prPrec i 6 (concatD [prt 6 expr, doc (showString "("), prt 0 exprs, doc (showString ")")])
    AbsLatte.EELem _ expr id_ -> prPrec i 6 (concatD [prt 6 expr, doc (showString "."), prt 0 id_])
    AbsLatte.ENewArr _ type_ expr -> prPrec i 6 (concatD [doc (showString "new"), prt 0 type_, doc (showString "["), prt 0 expr, doc (showString "]")])
    AbsLatte.ENew _ type_ -> prPrec i 6 (concatD [doc (showString "new"), prt 0 type_])
